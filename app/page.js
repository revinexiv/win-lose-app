...
export default function Home() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. Ambil Watchlist (ID yang mau dipantau)
      ...
      // 2. Tarik History (Anti-Limit 100rb+ Data)
      let allHistory = []
      let from = 0
      let to = 999
      let hasMore = true
      ...
      while (hasMore) {
        const { data, error } = await supabase
          .from('coin_history')
          .select('TO, Coin, Info')
          .eq('user_id', user?.id) // Filter per user biar ga nyampur
          .range(from, to)
        ...
        if (from > 500000) break // Safety limit 500rb baris
      }

      // 3. Hitung Manual (Logic Paling Akurat)
      ...
      for (let i = 0; i < allHistory.length; i++) {
        ...
        // HANYA proses ID yang ada di Watchlist
        if (monitoredIds.includes(rawName)) {
          ...
          if (!summaryMap.has(rawName)) {
            summaryMap.set(rawName, { name: rawName, total: 0, count: 0 })
          }
      ...
      // 4. Filter: Cuma tampilin yang ADA transaksi (count > 0)
  ...
  return (
    <div style={{ padding: '40px', backgroundColor: '#020617', backgroundImage: 'radial-gradient(circle at 50% 50%, #1e293b 0%, #020617 100%)', color: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px', borderBottom: '1px solid #1e293b', paddingBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '32px', fontWeight: '900', color: '#38bdf8', margin: 0 }}>CLouds Monitor V1</h1>
          <p style={{ color: '#94a3b8', fontSize: '14px', marginTop: '5px' }}>
            Tracking: <span style={{ color: '#00ff88', fontWeight: 'bold' }}>{rekap.length} Protocols with Transactions</span>
          </p>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '100px' }}>
          <p style={{ color: '#38bdf8', letterSpacing: '2px' }}>SYNCING LIVE DATABASE...</p>
        </div>
      ) : (
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '30px', display: 'flex', justifyContent: 'center', gap: '20px', alignItems: 'center' }}>
            <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} style={{ padding: '10px 20px', borderRadius: '8px', cursor: 'pointer' }}>PREV</button>
            <span style={{ color: '#38bdf8' }}>Page {currentPage} of {totalPages || 1}</span>
            <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage(prev => prev + 1)} style={{ padding: '10px 20px', borderRadius: '8px', cursor: 'pointer' }}>NEXT</button>
          </div>
        </div>
      )}
    </div>
